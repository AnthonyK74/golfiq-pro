export function calculateCourseFit(player, courseDNA = null) {
  ...
  const courseScore =
    driving * ((weights.offTee + weights.accuracy) / 100) +
    approach * (weights.approach / 100) +
    shortGame * ((weights.aroundGreen + weights.scrambling) / 100) +
    putting * (weights.putting / 100);
  ...
  if (score >= 95) recommendation = "Elite";
  else if (score >= 90) recommendation = "Excellent";
  ...
  let reason = `Strength: ${strongest.name}`;

  if (strongest.value >= 90 && second.value >= 85) {
    reason = `Elite ${strongest.name.toLowerCase()} and ${second.name.toLowerCase()}`;
  } else if (strongest.value >= 90) {
    reason = `Elite ${strongest.name.toLowerCase()}`;
  } else if (form >= 90) {
    reason = `Excellent recent form`;
  } else if (consistency >= 90) {
    reason = `Extremely consistent`;
  }
